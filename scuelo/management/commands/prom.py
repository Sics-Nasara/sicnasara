...
class Command(BaseCommand):
    help = 'Promotion automatique avec données élèves enrichies dans Excel, adapté au modèle Eleve'
    ...
    def get_classe_suivante_manuel(self, classe_actuelle):
        ...
        if classe_nom == fin_cycle:
            ...
            for ecole in ecoles_meme_ville:
                ...
                if classe_suivante:
                    self.stdout.write(f"[DEBUG] Promotion fin cycle : {classe_actuelle.nom} vers {classe_suivante.nom} dans {ecole.nom}")
                    return classe_suivante
            self.stdout.write(f"[DEBUG] Pas de classe suivante trouvée en fin de cycle pour {classe_actuelle.nom}")
            return None

        nom_suivant = self.PROMOTIONS_FIXES.get(type_ecole, {}).get(classe_nom)
        if not nom_suivant:
            self.stdout.write(f"[DEBUG] Pas de promotion fixe trouvée pour {classe_nom} de type {type_ecole}")
            return None
        ...
        if classe_suivante:
            self.stdout.write(f"[DEBUG] Promotion prévue : {classe_actuelle.nom} vers {classe_suivante.nom} dans {ecole_suivante.nom}")
        else:
            self.stdout.write(f"[DEBUG] Classe suivante '{nom_suivant}' introuvable dans {ecole_suivante.nom}")

        return classe_suivante

    def handle(self, *args, **options):
        dry_run = True  # mettre à False pour execution réelle
